...
"""
from turtle_tc import *; from turtle_tc import 形狀類, 龜類, 幕類, 主迴圈, 向量類 as 向量類
...
def 主函數():
    ...
    gs = 重力系統類()
    日 = 星類(1000000, 向量類(-250,0), 向量類(0,-0.35), gs, "circle")
    日.顏色(黃)
    日.筆粗(1.8)
    日.提筆()
    地球 = 星類(5000, 向量類(450,0), 向量類(0,70), gs, "planet")
    地球.筆色(綠)
    地球.形狀大小(0.8)

    rm=12.0583
    vm=(8.0*5000/rm)**.5
    月球 = 星類(1, 向量類(450+rm,0), 向量類(0,70+vm), gs, "planet")
    月球.筆色(藍)
    月球.形狀大小(0.5)
    gs.起始化()
    gs.開始()
